...
const prepareAccessToken = async (info) => {
	...
	const accessExp = date.addHours(new Date(), TOKEN.TOKEN_EXPIRE_HOURS);
	await redis.setex(redisAccessId, accessExp.getTime(), true);
...
const prepareRefreshToken = async (info) => {
	...
	const refreshExp = date.addWeeks(
		new Date(),
		TOKEN.REFRESH_TOKEN_EXPIRE_WEEKS
	);
	await redis.setex(redisRefreshId, refreshExp.getTime(), true);
